...
public class CreateTableGeneratorInterbase extends CreateTableGenerator {

	Logger LOGGER = LoggerFactory.getLogger(CreateTableGeneratorInterbase.class);
    ...
    public Sql[] generateSql(CreateTableStatement statement, Database database, SqlGeneratorChain sqlGeneratorChain) {
        ...
        while (columnIterator.hasNext()) {
            ...
            if (isAutoIncrementColumn) {
              // (database.getShortName()+" does not support autoincrement columns as request for "+(database.escapeTableName(statement.getCatalogName(), statement.getSchemaName(), statement.getTableName())));
               LOGGER.error(database.getShortName()+" does not support autoincrement columns as request for "+(database.escapeTableName(statement.getCatalogName(), statement.getSchemaName(), statement.getTableName())));
            }
        ...
        if (statement.getPrimaryKeyConstraint() != null && statement.getPrimaryKeyConstraint().getColumns().size() > 0) {
        	...
        	buffer.append(statement.getTableName() + "PK");
        ...
        for (ForeignKeyConstraint fkConstraint : statement.getForeignKeyConstraints()) {
            ...
            referencesString = fkConstraint.getReferencedTableName() + "(" + fkConstraint.getReferencedColumnNames() + ")";
            
            buffer.append(" FOREIGN KEY (")
                    .append(database.escapeColumnName(statement.getCatalogName(), 
                    		statement.getSchemaName(), statement.getTableName(), 
                    		fkConstraint.getColumn()))
                    .append(") REFERENCES ")
                    .append(referencesString);

//            if (fkConstraint.isUpdateCascade()) {
